...
public class Quickstart {
    ...
    public static void main(String[] europium) {
        ...
        JsonObject[] boundingBox = new JsonObject[] { gson.fromJson(mapView.get("TopLeft"), JsonObject.class),
                gson.fromJson(mapView.get("BottomRight"), JsonObject.class) };
        double[] topLeft = new double[] { gson.fromJson(boundingBox[1].get("Latitude"), double.class),
                gson.fromJson(boundingBox[0].get("Longitude"), double.class) };
        double[] bottomRight = new double[] { gson.fromJson(boundingBox[0].get("Latitude"), double.class),
                gson.fromJson(boundingBox[1].get("Longitude"), double.class) };

        System.out.printf("Top left: %f %f\nBottom Right: %f %s", topLeft[0], topLeft[1], bottomRight[0],
                bottomRight[1]);

        // JsonObject location = gson.fromJson(location.getAsJsonObject(),
        // JsonObject.class);
